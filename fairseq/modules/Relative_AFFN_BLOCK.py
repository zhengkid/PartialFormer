...
class RelativeAFFNBlock(nn.Module):
    ...
    def __init__(
        ...
        self.ff1 = quant_noise(
            nn.Linear(self.head_dim,
                      args.hidden_ratio * self.head_dim if not decoder else args.decoder_hidden_ratio * self.head_dim,
                      bias=bias), q_noise, qn_block_size
        )
        self.ff2 = quant_noise(
            nn.Linear(args.hidden_ratio * self.head_dim if not decoder else args.decoder_hidden_ratio * self.head_dim,
                      self.head_dim, bias=bias), q_noise, qn_block_size
        )
    ...
    def forward(
        ...
        if saved_state is not None:
            ...
            key_padding_mask = RelativeAFFNBlock._append_prev_key_padding_mask(
