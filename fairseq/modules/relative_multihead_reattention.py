...
class RelativeMultiheadReAttention(nn.Module):
    ...
    def __init__(
        ...
        '''
        self.reatten_matrix_weight = nn.Parameter(torch.Tensor(1024, 1024))
        self.reatten_matrix_bias = nn.Parameter(torch.Tensor(1024))
        nn.init.xavier_uniform_(self.reatten_matrix_weight)
        nn.init.constant_(self.reatten_matrix_bias, 0.)
        '''
        self.reatten_matrix = nn.Conv2d(self.num_heads,self.num_heads, 1, 1)
        self.var_norm = nn.BatchNorm2d(self.num_heads)
        #self.var_norm = LayerNorm(self.num_heads)
        self.layer_norm = LayerNorm(self.head_dim)
    ...
    def forward(
        ...
        attn_probs = self.var_norm(self.reatten_matrix(attn_probs))
        attn_probs = attn_probs.view(bsz * self.num_heads, tgt_len, src_len)
        assert v is not None
        # key only mode
        if self.k_only:
    ...
    def _generate_relative_positions_matrix(self, length, max_relative_length, incremental_state):
        ...
        distance_mat_clipped = torch.clamp(distance_mat, -max_relative_length, max_relative_length).cuda()
