...
class MultiheadAttentionFFNConv(nn.Module):
    ...
    def __init__(
        ...
        self.ff1 = quant_noise(nn.Linear(self.head_dim, args.decoder_hidden_ratio * self.head_dim, bias=bias), q_noise,
                               qn_block_size)
        self.ff2 = quant_noise(nn.Linear(args.decoder_hidden_ratio * self.head_dim, self.head_dim, bias=bias), q_noise,
                               qn_block_size)
        ...
        self.decoder_head_interact = args.decoder_head_interact
        if self.decoder_head_interact:
            self.head_ff1 = quant_noise(
                nn.Linear(self.num_heads, args.decoder_head_hidden_ratio * self.num_heads, bias=bias), q_noise,
                qn_block_size
            )
            self.head_ff2 = quant_noise(
                nn.Linear(args.decoder_head_hidden_ratio * self.num_heads, self.num_heads, bias=bias),
                q_noise,
                qn_block_size
                )
    ...
    def forward(
        ...
        if saved_state is not None:
            ...
            key_padding_mask = MultiheadAttentionFFNConv._append_prev_key_padding_mask(
