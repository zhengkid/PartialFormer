...
def collate(
    ...
    #sort_order = torch.load("/mnt/zhengtong/code/ODE-transformer/fairseq-0.10.2-master-375b3747a2b1b6576462d828179362749229cf77/order.pt")
        ...
        for i in range(tree.shape[0]):
            ...
            for j in range(tree.shape[1]):
                if tree_list[i][j] >= 5:
                    ...
                    mapping[i, j, -1] =1
        return mapping.half().to(tree.device)
    #a = src_dict.string(src_tokens[0])
    #print(a)
    def gen_local_matrix(seq_len):
        len_matrix = torch.arange(0, seq_len)
        len_matrix = len_matrix.repeat(seq_len, 1)
        len_matrix = len_matrix - len_matrix.transpose(0,1)
        return len_matrix + torch.eye(seq_len)
    ...
    local_mapping = gen_local_matrix(tree.size(1))
...
class DPTreeLocalPhraseLanguagePairDataset(FairseqDataset):
    """
