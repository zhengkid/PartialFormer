...
def filter_paired_tree_dataset_indices_by_size(src_sizes, tgt_sizes, tree_sizes, indices, max_sizes):
    """Filter a list of sample indices. Remove those that are longer
        than specified in max_sizes.
    ...
    if max_sizes is None:
        return indices, []
    if type(max_sizes) in (int, float):
        max_src_size, max_tgt_size, max_tree_size = max_sizes, max_sizes, max_sizes
    else:
        max_src_size, max_tgt_size, max_tree_size = max_sizes[0], max_sizes[1], max_sizes[0]
    if tgt_sizes is None:
        ignored = indices[src_sizes[indices] > max_src_size]
    else:
        ignored = indices[
            (src_sizes[indices] > max_src_size) | (tgt_sizes[indices] > max_tgt_size) |(tree_sizes[indices] > max_tree_size)
        ]
    if len(ignored) > 0:
        if tgt_sizes is None:
            indices = indices[src_sizes[indices] <= max_src_size]
            ...
            indices = indices[
                (src_sizes[indices] <= max_src_size)
                & (tgt_sizes[indices] <= max_tgt_size)
                & (tree_sizes[indices] <= max_tree_size)
            ]
    return indices, ignored.tolist()
