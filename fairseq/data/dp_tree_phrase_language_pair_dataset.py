            ...
            for j in range(0, phrase.shape[1], 2):
                if phrase_mapping[i][j] >= 5:
                    if (phrase_mapping[i][j]-5) >= size1 and (phrase_mapping[i][j+1]-5) >=size2:
                        ...
                        mapping[i][phrase_mapping[i][j]-5+diff[i] if (phrase_mapping[i][j]-5+diff[i]) < size1 else 0][phrase_mapping[i][j+1]-5+diff[i] if (phrase_mapping[i][j+1]-5+diff[i]) < size2 else 0] = 1
                        mapping[i][phrase_mapping[i][j+1]-5+diff[i] if (phrase_mapping[i][j+1]-5+diff[i]) < size1 else 0][phrase_mapping[i][j]-5+diff[i] if (phrase_mapping[i][j]-5+diff[i]) < size2 else 0] = 1
                else:
                    continue
        ...
        for i in range(tree.shape[0]):
            ...
            for j in range(tree.shape[1]):
                if tree_list[i][j] >= 5:
                    ...
                    mapping[i, j, -1] =1
        return mapping.half().to(tree.device)
    bpe2word_scale = gen_group_matrix(tree).numpy()
    bpe2word_wo_scale = gen_group_matrix_gcn(tree)
    phrase_mapping = gen_phrase_matrix(phrase_mapping)
    phrase_mapping = np.matmul(np.matmul(bpe2word_scale, phrase_mapping), bpe2word_scale.swapaxes(1,2))
    phrase_mapping = phrase_mapping + np.eye(phrase_mapping.shape[1])
    #print(phrase_mapping)
    for i in range(phrase_mapping.shape[0]):
        graph = phrase_mapping[i]
        deg = np.sum(graph,axis=-1)
        deg = np.diag((deg ** (-0.5)))
        phrase_mapping[i] = np.matmul(np.matmul(deg, graph),deg)
    phrase_mapping = torch.from_numpy(phrase_mapping).half().to(tree.device)
    bpe2word_scale = torch.from_numpy(bpe2word_scale).half().to(tree.device)
...
class DPTreePhraseLanguagePairDataset(FairseqDataset):
    """
