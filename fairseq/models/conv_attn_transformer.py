...
@register_model("convattn_transformer")
class ConvAttn_TransformerModel(FairseqEncoderDecoderModel):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--conv-layers", type=int, metavar="N", help="num encoder layers"
        )

        parser.add_argument(
            "--attn-layers", type=int, metavar="N", help="num encoder layers"
        )
    ...
    def build_model(cls, args, task):
        ...
        encoder = ConvAttn_TransformerEncoder(args, src_dict, encoder_embed_tokens)
        decoder = ConvAttn_TransformerDecoder(args, tgt_dict, decoder_embed_tokens)
        return ConvAttn_TransformerModel(encoder, decoder)


class ConvAttn_TransformerEncoder(FairseqEncoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.layers.extend(
            [
                ConvAttn_TransformerEncoderLayer(
        ...
        self.layers.extend(
            [
                ...
                for i in range(args.attn_layers)
            ]
        )
...
class ConvAttn_TransformerDecoder(FairseqIncrementalDecoder):
    """
    ...
    def __init__(
        ...
        self.layers.extend(
            [
                ConvAttn_TransformerDecoderLayer(
...
class ConvAttn_TransformerEncoderLayer(nn.Module):
    """Encoder layer block.
...
class ConvAttn_TransformerDecoderLayer(nn.Module):
    """Decoder layer block.
...
@register_model_architecture("convattn_transformer", "convattn_transformer")
def base_architecture(args):
    ...
    args.encoder_kernel_size_list = getattr(
        args, "encoder_kernel_size_list", [3, 3, 3, 7, 7, 7, 15, 15, 15, 31, 31, 31, 31, 31]
    )
    ...
    assert (
        len(args.encoder_kernel_size_list) == args.conv_layers
    ...
    args.conv_layers = getattr(args, "conv_layers", 7)
    args.attn_layers = getattr(arg, "attn_layers", 6)



@register_model_architecture("convattn_transformer", "convattn_transformer_iwslt_de_en")
def convattn_transformer_iwslt_de_en(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
...
@register_model_architecture("convattn_transformer", "convattn_transformer_t2t_wmt_en_de")
def convattn_transformer_t2t_wmt_en_de(args):
...
@register_model_architecture("convattn_transformer", "convattn_transformer_wmt_en_de_big")
def convattn_transformer_wmt_en_de_big(args):
    args.attention_dropout = getattr(args, "attention_dropout", 0.1)
...
@register_model_architecture("convattn_transformer", "convattn_transformer_wmt_en_fr_big")
def convattn_transformer_wmt_en_fr_big(args):
    args.dropout = getattr(args, "dropout", 0.1)
    convattn_transformer_wmt_en_de_big(args)


@register_model_architecture("convattn_transformer", "convattn_transformer_wmt_zh_en_big")
def convattn_transformer_wmt_zh_en_big(args):
